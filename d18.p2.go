...
// copying the robot destination slice in a sequence
func copyDest(des []byte) (cdes []byte) {
	cdes = make([]byte, len(des))
	for i,v := range des {
		cdes[i] = v
	}
	return
...
type path struct {
	rob int             // the number of the robot (not determined at initial parsing)
...
func detectPaths(org byte) (prPaths []path) {
	...
	for _, nP := range nPs {
		...
		trc[origin] = true
		trc[nP]     = true
		paths = append(paths, path{ln:1, org:org, des:255, drs:0, cur:nP, trc:trc, rob:-1})
	}
	...
	for (curPth < len(paths)) {
		...
		if tst2 && key != org {
			...
			paths = append(paths, path{ln:pth.ln, org:pth.org, des:key, drs:pth.drs, cur:pth.cur, trc:trc, rob:-1})
		}
		...
		if len(nPs) > 1 {
			...
			for i := 1; i < len(nPs); i++ {
				trc := copyTrace(pth.trc)
				trc[nPs[i]] = true
				paths = append(paths, path{ln:pth.ln+1, org:pth.org, des:255, drs:pth.drs, cur:nPs[i], trc:trc, rob:-1})
			}
		}
...
func dumpSGraph() {
	...
	for _, ss := range sGraph {
		fmt.Printf("> Sequence length %4d with keys %0" + strconv.Itoa(int(maxKey)+1) + "b\n", ss.len, ss.keys)
	}
	fmt.Println()
}
...
type sequ struct {
	name  string // a name indicating the visited keys in sequence (for debugging)
	keys  int    // an int with bits represnting keys (bit0 = 'a')
	dest  []byte // the current destinations of all robots 
	len   int    // the total length of the sequence
}

// thie helps keeping track which locations where already visited with the same keys in hand
type visK struct {
	loc   byte
	keys  int
}

// this builds valid sequences of paths and returns the shortest path
func buildSequences () (short int) {
	...
	sGraph = []sequ{}
	for i := 0; i < 4; i++ {
		for _, pth := range pGraph[byte(200+i)] {
			if pth.drs == 0 {
				des := []byte{200,201,202,203}
				des[i] = pth.des
				sGraph = append(sGraph, sequ{name:string(pth.des + 'a'), keys:(1 << pth.des), dest:des, len:pth.ln})
			}
		}
	}
	...
	for (currSq < len(sGraph)) {
		...
		// loop through all four robots
		for i := 0; i < 4; i++ {
			ptmp    := pGraph[sGraph[currSq].dest[i]]
			for j := 0; j < len(ptmp); j++ {
				ptmp[j].rob = i
			}		
			allPaths = append(allPaths, ptmp...)		
		}
		...
		// branch if more than one next path is possible
		if len(paths) > 1 {
			for i := 1; i < len(paths); i++ {
				des   := copyDest(sGraph[currSq].dest) 
				des[paths[i].rob] = paths[i].des
		...
		if len(paths) > 0 {
			...
			des                 := copyDest(sGraph[currSq].dest)
			des[paths[0].rob]    = paths[0].des
			sGraph[currSq].dest  = des
...
func main() {
	start := time.Now()
	parse(readTxtFile("d18.p2." + os.Args[1] + ".txt"))

	// fmt.Println(keys, doors, maxKey)
	pGraph      = make([][]path, 256)
	for i := 0; i < 4; i++ {
		pGraph[200+i] = detectPaths(byte(200+i))		
	}
	for i := byte(0); i <= maxKey; i++ {
